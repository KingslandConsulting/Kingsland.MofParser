...
namespace Kingsland.MofParser.UnitTests.Lexing
{
    ...
    public sealed class LexerAssert
    {
        ...
        public static void AreEqual(List<SyntaxToken> expectedTokens, List<SyntaxToken> actualTokens)
        {
            ...
            {
                LexerAssert.AreEqualInternal(expectedTokens[i], actualTokens[i], i);
            }
            Assert.AreEqual(expectedTokens.Count, actualTokens.Count, "expected and actual are different lengths");
        }

        private static void AreEqualInternal(SyntaxToken expectedToken, SyntaxToken actualToken, int index = -1)
        {
            ...
            switch (expectedToken)
            {
                case AliasIdentifierToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((AliasIdentifierToken)expectedToken, (AliasIdentifierToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case AttributeCloseToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((AttributeCloseToken)expectedToken, (AttributeCloseToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case AttributeOpenToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((AttributeOpenToken)expectedToken, (AttributeOpenToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case BlockCloseToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((BlockCloseToken)expectedToken, (BlockCloseToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case BlockOpenToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((BlockOpenToken)expectedToken, (BlockOpenToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case BooleanLiteralToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((BooleanLiteralToken)expectedToken, (BooleanLiteralToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case ColonToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((ColonToken)expectedToken, (ColonToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case CommaToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((CommaToken)expectedToken, (CommaToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case CommentToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((CommentToken)expectedToken, (CommentToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case DotOperatorToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((DotOperatorToken)expectedToken, (DotOperatorToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case EqualsOperatorToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((EqualsOperatorToken)expectedToken, (EqualsOperatorToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case IdentifierToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((IdentifierToken)expectedToken, (IdentifierToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                ...
                case NullLiteralToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((NullLiteralToken)expectedToken, (NullLiteralToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case ParenthesisCloseToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((ParenthesisCloseToken)expectedToken, (ParenthesisCloseToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case ParenthesisOpenToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((ParenthesisOpenToken)expectedToken, (ParenthesisOpenToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case PragmaToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((PragmaToken)expectedToken, (PragmaToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case RealLiteralToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((RealLiteralToken)expectedToken, (RealLiteralToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case StatementEndToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((StatementEndToken)expectedToken, (StatementEndToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case StringLiteralToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((StringLiteralToken)expectedToken, (StringLiteralToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                case WhitespaceToken _:
                    Assert.IsTrue(
                        TokenAssert.AreEqual((WhitespaceToken)expectedToken, (WhitespaceToken)actualToken),
                        LexerAssert.GetAssertErrorMessage($"actual token does not match expected token", index)
                    );
                    break;
                default:
